...
class KsListExport(http.Controller):
    ...
    def base(self, data):
        ...
        if ks_export_boolean:
            item = request.env['ks_dashboard_mtn.item'].browse(int(item_id))
            ...
            if item._context.get('ksDateFilterSelection', False):
                ...
                ks_date_filter_selection = item.ks_dashboard_mtn_board_id.ks_date_filter_selection
                item = item.with_context(ksDateFilterStartDate=item.ks_dashboard_mtn_board_id.ks_dashboard_start_date)
                item = item.with_context(ksDateFilterEndDate=item.ks_dashboard_mtn_board_id.ks_dashboard_end_date)
        ...
        for dataset in list_data['data_rows']:
            ...
            for ks_count, val in enumerate(dataset['data']):
                if isinstance(val, (float, int)):
                    if val >= 0:
                        try:
                            ks_precision = item.sudo().env.ref('ks_dashboard_mtn.ks_dashboard_mtn_precision').digits
                        except Exception as e:
                            ks_precision = 2
...
class KsListExcelExport(KsListExport, http.Controller):
    ...
    @http.route('/ks_dashboard_mtn/export/list_xls', type='http', auth="user")
    @serialize_exception
    def index(self, data):
...
class KsListCsvExport(KsListExport, http.Controller):
    ...
    @http.route('/ks_dashboard_mtn/export/list_csv', type='http', auth="user")
    @serialize_exception
    def index(self, data):
        return self.base(data)
