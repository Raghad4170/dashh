...
import datetime
import json
from odoo.addons.ks_dashboard_mtn.common_lib.ks_date_filter_selections import ks_get_date, ks_convert_into_local, \
    ks_convert_into_utc
...
class KsDashboardmtnBoard(models.Model):
    _name = 'ks_dashboard_mtn.board'
    _description = 'Dashboard mtn'

    name = fields.Char(string="اسم لوحة المعلومات", required=True, size=35)
    ks_dashboard_items_ids = fields.One2many('ks_dashboard_mtn.item', 'ks_dashboard_mtn_board_id',
                                             string='عناصر لوحة المعلومات')
    ks_dashboard_menu_name = fields.Char(string="اسم القائمة")
    ks_dashboard_top_menu_id = fields.Many2one('ir.ui.menu',
                                               domain="['|',('action','=',False),('parent_id','=',False)]",
                                               string="إظهار ضمن القائمة",
    ...
    ks_dashboard_active = fields.Boolean(string="نشط", default=True)
    ks_dashboard_group_access = fields.Many2many('res.groups', string="صلاحيات الوصول")

    # DateFilter Fields
    ks_dashboard_start_date = fields.Datetime(string="تاريخ البدء")
    ks_dashboard_end_date = fields.Datetime(string="تاريخ الانتهاء")
    ks_date_filter_selection = fields.Selection([
        ...
        ('l_custom', 'تصفية مخصص'),
    ], default='l_none', string="التاريخ الافتراضي")
    ...
    ks_dashboard_default_template = fields.Many2one('ks_dashboard_mtn.board_template',
                                                    default=lambda self: self.env.ref('ks_dashboard_mtn.ks_blank',
                                                                                      False),
    ...
    ks_set_interval = fields.Selection([
        ...
        ('300000', '5 دقائق'),
        ('600000', '10 دقائق'),
    ], string="الفاصل الزمني للتحديث الافتراضي", help="Update Interval for new items only")
    ks_dashboard_menu_sequence = fields.Integer(string="تسلسل القائمة", default=10,
                                                help="Smallest sequence give high priority and Highest sequence give "
                                                     "low priority")
    ks_child_dashboard_ids = fields.One2many('ks_dashboard_mtn.child_board', 'ks_dashboard_mtn_id')
    ks_dashboard_defined_filters_ids = fields.One2many('ks_dashboard_mtn.board_defined_filters',
                                                       'ks_dashboard_board_id',
                                                       string='مرشحات محددة مسبقا للوحة المعلومات')
    ks_dashboard_custom_filters_ids = fields.One2many('ks_dashboard_mtn.board_custom_filters',
                                                      'ks_dashboard_board_id',
                                                      string='مرشحات لوحة المعلومات المخصصة')
    multi_layouts = fields.Boolean(string='تمكين تخطيطات لوحات المعلومات المتعددة',
        ...
        if record.ks_dashboard_default_template and record.ks_dashboard_default_template.ks_item_count:
            ...
            for item_data in template_data:
                if record.ks_dashboard_default_template.ks_template_type == 'ks_custom':
                    dashboard_item = self.env['ks_dashboard_mtn.item'].browse(int(item_data)).copy(
                        {'ks_dashboard_mtn_board_id': record.id})
                    ks_gridstack_config[dashboard_item.id] = template_data[item_data]
                else:
                    dashboard_item = self.env.ref(item_data['item_id']).copy({'ks_dashboard_mtn_board_id': record.id})
    ...
    def write(self, vals):
        ...
        record = super(KsDashboardmtnBoard, self).write(vals)
        for rec in self:
            if 'ks_dashboard_menu_name' in vals:
                if self.env.ref('ks_dashboard_mtn.ks_my_default_dashboard_board') and self.env.ref(
                        'ks_dashboard_mtn.ks_my_default_dashboard_board').sudo().id == rec.id:
                    if self.env.ref('ks_dashboard_mtn.board_menu_root', False):
                        self.env.ref('ks_dashboard_mtn.board_menu_root').sudo().name = vals['ks_dashboard_menu_name']
                else:
                    rec.ks_dashboard_menu_id.sudo().name = vals['ks_dashboard_menu_name']
            if 'ks_dashboard_group_access' in vals:
                if self.env.ref('ks_dashboard_mtn.ks_my_default_dashboard_board').id == rec.id:
                    if self.env.ref('ks_dashboard_mtn.board_menu_root', False):
                        self.env.ref('ks_dashboard_mtn.board_menu_root').groups_id = vals['ks_dashboard_group_access']
                else:
    ...
    def unlink(self):
        if self.env.ref('ks_dashboard_mtn.ks_my_default_dashboard_board').id in self.ids:
        ...
        res = super(KsDashboardmtnBoard, self).unlink()
        return res

    def ks_get_grid_config(self):
        default_grid_id = self.env['ks_dashboard_mtn.child_board'].search(
            [['id', 'in', self.ks_child_dashboard_ids.ids], ['company_id', '=', self.env.company.id],
             ['board_type', '=', 'default']])

        if not default_grid_id:
            default_grid_id = self.env['ks_dashboard_mtn.child_board'].create({
                "ks_gridstack_config": self.ks_gridstack_config,
                "ks_dashboard_mtn_id": self.id,
    ...
    def ks_fetch_dashboard_data(self, ks_dashboard_id, ks_item_domain=False):
        ...
        has_group_ks_dashboard_manager = self.env.user.has_group('ks_dashboard_mtn.ks_dashboard_mtn_group_manager')
        ...
        if self.env['ks_dashboard_mtn.child_board'].search(
            ...
            selecred_rec = self.env['ks_dashboard_mtn.child_board'].search(
            ...
            for rec in self.env['ks_dashboard_mtn.child_board'].search_read(
        ...
        try:
            items = self.ks_dashboard_items_ids.search(
                [['ks_dashboard_mtn_board_id', '=', ks_dashboard_id]] + ks_item_domain).ids
    ...
    def ks_fetch_item(self, item_list, ks_dashboard_id, params={}):
        ...
        items = {}
        item_model = self.env['ks_dashboard_mtn.item']
    ...
    def ks_fetch_item_data(self, rec, params={}):
        ...
        """
        try:
            ks_precision = self.sudo().env.ref('ks_dashboard_mtn.ks_dashboard_mtn_precision')
        ...
        item = {
            ...
            'ks_dashboard_id': rec.ks_dashboard_mtn_board_id.id,
            'ks_icon': rec.ks_icon,
    ...
    def ks_view_items_view(self):
        ...
        return {
            ...
            'res_model': 'ks_dashboard_mtn.item',
            'view_mode': 'tree,form',
            'view_type': 'form',
            'views': [(False, 'tree'), (False, 'form')],
            'type': 'ir.actions.act_window',
            'domain': [('ks_dashboard_mtn_board_id', '!=', False)],
            'search_view_id': self.env.ref('ks_dashboard_mtn.ks_item_search_view').id,
            'context': {
                'search_default_ks_dashboard_mtn_board_id': self.id,
                'group_by': 'ks_dashboard_mtn_board_id',
            },
    ...
    def ks_export_item(self, item_id):
        return {
            'ks_file_format': 'ks_dashboard_mtn_item_export',
    ...
    def ks_export_item_data(self, rec):
        ...
        val = str(rec.id)
        selecred_rec = self.env['ks_dashboard_mtn.child_board'].search(
            [['id', 'in', rec.ks_dashboard_mtn_board_id.ks_child_dashboard_ids.ids], ['ks_active', '=', True],
             ['company_id', '=', self.env.company.id]], limit=1)
        if rec.ks_dashboard_mtn_board_id.ks_gridstack_config:
            keys_data = json.loads(rec.ks_dashboard_mtn_board_id.ks_gridstack_config)
        elif selecred_rec:
            keys_data = json.loads(selecred_rec.ks_gridstack_config)
        elif rec.ks_dashboard_mtn_board_id.ks_child_dashboard_ids[0].ks_gridstack_config:
            keys_data = json.loads(rec.ks_dashboard_mtn_board_id.ks_child_dashboard_ids[0].ks_gridstack_config)
        else:
    ...
    def ks_open_import(self, **kwargs):
        action = self.env['ir.actions.act_window']._for_xml_id('ks_dashboard_mtn.ks_import_dashboard_action')
        return action

    def ks_open_setting(self, **kwargs):
        action = self.env['ir.actions.act_window']._for_xml_id('ks_dashboard_mtn.board_form_tree_action_window')
    ...
    def ks_delete_dashboard(self):
        if str(self.id) in self.ks_dashboard_default_template:
            ...
            return {
                ...
                'name': "Dashboard mtn",
                'res_model': 'ks_deshboard_mtn.board',
    ...
    def ks_import_item(self, dashboard_id, **kwargs):
        ...
        if 'ks_file_format' in ks_dashboard_file_read and ks_dashboard_file_read[
            ...
            raise ValidationError(_("Current Json File is not properly formatted according to Dashboard mtn Model."))

        item['ks_dashboard_mtn_board_id'] = int(dashboard_id)
    ...
    def ks_dashboard_export(self, ks_dashboard_ids, **kwargs):
        ...
        for ks_dashboard_id in ks_dashboard_ids:
            ...
            ks_dashboard_export_data = {
                'ks_file_format': 'ks_dashboard_mtn_export_file',
        ...
        for data in ks_dashboard_data:
            if not all(key in data for key in ks_dashboard_key):
                raise ValidationError(
                    _("Current Json File is not properly formatted according to Dashboard mtn Model."))
            ...
            vals = {
                ...
                'ks_dashboard_top_menu_id': menu_id.id if menu_id else self.env.ref(
                    "ks_dashboard_mtn.board_menu_root").id,
                'ks_dashboard_active': True,
                'ks_gridstack_config': data['ks_gridstack_config'],
                'ks_dashboard_default_template': self.env.ref("ks_dashboard_mtn.ks_blank").id,
            ...
            if data['ks_item_data']:
                ...
                for item in data['ks_item_data']:
                    ...
                    item['ks_dashboard_mtn_board_id'] = dashboard_id.id
    ...
    def ks_create_item(self, item):
        ...
        ks_item = self.env['ks_dashboard_mtn.item'].create(item)
        ...
        if ks_goal_lines and len(ks_goal_lines) != 0:
            for line in ks_goal_lines:
                ...
                self.env['ks_dashboard_mtn.item_goal'].create(line)
            ...
            for line in ks_action_lines:
                ...
                if line['ks_item_action_field']:
                    ...
                    if ks_record_id:
                        ...
                        self.env['ks_dashboard_mtn.item_action'].create(line)
    ...
    def update_child_board(self, action, dashboard_id, data):
        ...
        if action == 'create':
            ...
            result = self.env['ks_dashboard_mtn.child_board'].create(data)
            result = result.id
        elif action == 'update':
    ...
    def ks_prepare_dashboard_domain(self):
        pre_defined_filter_ids = self.env['ks_dashboard_mtn.board_defined_filters'].search(
        ...
        for model_id in filter_model_ids:
            filter_ids = self.env['ks_dashboard_mtn.board_defined_filters'].search(
            ...
            for rec in filter_ids:
                ...
                if data.get(rec.ks_model_id.model) and rec.ks_domain:
                    ...
                    data[rec.ks_model_id.model] = {
                        ...
                        'item_ids': self.env['ks_dashboard_mtn.item'].search(
    ...
    def ks_prepare_dashboard_custom_domain(self):
        custom_filter_ids = self.env['ks_dashboard_mtn.board_custom_filters'].search(
