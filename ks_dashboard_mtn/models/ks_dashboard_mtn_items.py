...
from odoo.addons.ks_dashboard_mtn.common_lib.ks_date_filter_selections import ks_get_date, ks_convert_into_utc, \
...
class KsDashboardmtnItems(models.Model):
    _name = 'ks_dashboard_mtn.item'
    _description = 'Dashboard mtn items'

    name = fields.Char(string="الاسم", size=256, help="The item will be represented by this unique name.")
    ks_model_id = fields.Many2one('ir.model', string='النموذج',
    ...
    ks_dashboard_board_template_id = fields.Many2one('ks_dashboard_mtn.board_template', string="قالب لوحة المعلومات")
    ks_domain = fields.Char(string="الدومين", help="Define conditions for filter. ")

    ks_model_id_2 = fields.Many2one('ir.model', string='نموذج Kpi',
                                    domain="[('access_ids','!=',False),('transient','=',False),"
                                           "('model','not ilike','base_import%'),('model','not ilike','ir.%'),"
                                           "('model','not ilike','web_editor.%'),('model','not ilike','web_tour.%'),"
                                           "('model','!=','mail.thread'),('model','not ilike','ks_dash%'), ('model','not ilike','ks_to%')]")

    ks_model_name_2 = fields.Char(related='ks_model_id_2.model', string="اسم نموذج Kpi")

    # This field main purpose is to store %UID as current user id. Mainly used in JS file as container.
    ks_domain_temp = fields.Char(string="المجال البديل")
    grid_corners = fields.Char(string="زوايا الشبكة")
    ks_background_color = fields.Char(string="لون الخلفية",
                                      default="#ffffff,0.99", help=' Select the background color with transparency. ')
    ks_icon = fields.Binary(string="تحميل أيقونة", attachment=True)
    ks_default_icon = fields.Char(string="أيقونة", default="bar-chart", help='Select the icon to be displayed. ')
    ks_default_icon_color = fields.Char(default="#ffffff,0.99", string="لون الأيقونة",
                                        help='Select the icon to be displayed. ')
    ks_icon_select = fields.Char(string="خيار الأيقونات", default="Default", help='Choose the Icon option. ')
    ks_font_color = fields.Char(default="#ffffff,0.99", string="لون الخط", help='Select the font color. ')
    ks_dashboard_item_theme = fields.Char(string="الخلفية", default="white",
                                          help='Select the color theme for the display. ')
    ks_layout = fields.Selection([('layout1', 'تنسيق 1'),
                                  ('layout2', 'تنسيق 2'),
                                  ('layout3', 'تنسيق 3'),
                                  ('layout4', 'تنسيق 4'),
                                  ('layout5', 'تنسيق 5'),
                                  ('layout6', 'تنسيق 6'),
                                  ], default=('layout1'), required=True, string="تنسيق",
                                 help=' Select the layout to display records. ')
    ks_preview = fields.Integer(default=1, string="معاينة")
    ks_model_name = fields.Char(related='ks_model_id.model', string="اسم النموذج")

    ks_record_count_type_2 = fields.Selection([('count', 'العد'),
                                               ('sum', 'المجموع'),
                                               ('average', 'المتوسط')], string="نوع سجل Kpi", default="sum")
    ks_record_field_2 = fields.Many2one('ir.model.fields',
                                        domain="[('model_id','=',ks_model_id_2),('name','!=','id'),('name','!=','sequence'),('store','=',True),"
                                               ...
                                               "('ttype','=','monetary')]",
                                        string="حقل سجل Kpi")
    ks_record_count_2 = fields.Float(string="عدد سجلات مؤشرات الأداء الرئيسية", readonly=True, compute='ks_get_record_count_2',
                                     compute_sudo=False)
    ks_record_count_type = fields.Selection([('count', 'العد'),
                                             ('sum', 'المجموع'),
                                             ('average', 'المتوسط')], string="نوع السجل", default="count",
                                            help="Type of record how record will show as count,sum and average of the record")
    ks_record_count = fields.Float(string="عدد السجلات", compute='ks_get_record_count', readonly=True,
                                   compute_sudo=False)
    ks_record_field = fields.Many2one('ir.model.fields',
                                      domain="[('model_id','=',ks_model_id),('name','!=','id'),('store','=',True),'|',"
                                             ...
                                             "('ttype','=','monetary')]",
                                      string="سجل الحقل")
    ks_record_data_limit_visibility = fields.Boolean(string="رؤية بيانات حد السجل",
    ...
    ks_date_filter_selection = fields.Selection([
        ...
        ('t_year', 'هذه السنة'),
        ('n_day', 'اليوم التالي'),
        ('n_week', 'الاسبوع القادم'),
        ('n_month', 'الشهر القادم'),
        ('n_quarter', 'الربع القادم'),
        ('n_year', 'السنة القادمة'),
        ('ls_day', 'الأمس'),
        ('ls_week', 'الاسبوع الماضي'),
        ('ls_month', 'الشهر الماضي'),
        ('ls_quarter', 'الربع الماضي'),
        ('ls_year', 'السنة الماضية'),
        ('l_week', 'اخر 7 ايام'),
        ('l_month', 'اخر 30 يومًا'),
        ('l_quarter', 'اخر 90 يومًا'),
        ('l_year', 'اخر 365 يومًا'),
        ('ls_past_until_now', 'من الماضي حتى الآن'),
        ('ls_pastwithout_now', ' الماضي باستثناء اليوم'),
        ('n_future_starting_now', 'المستقبل يبدأ الآن'),
        ('n_futurestarting_tomorrow', 'المستقبل يبدأ غدا'),
        ('l_custom', 'تصفية مخصص'),
    ], string="التاريخ الافتراضي", default="l_none", required=True,
    ...
    ks_date_filter_field = fields.Many2one('ir.model.fields',
                                           domain="[('model_id','=',ks_model_id),('store','=',True),'|',('ttype','=','date'),"
                                                  "('ttype','=','datetime')]",
                                           string=" تصفية التاريخ",
                                           help='Select the field for which Date Filter should be applicable.')

    ks_item_start_date = fields.Datetime(string="تاريخ البدء")
    ks_item_end_date = fields.Datetime(string="تاريخ الانتهاء")

    ks_date_filter_field_2 = fields.Many2one('ir.model.fields',
                                             domain="[('model_id','=',ks_model_id_2),('store','=',True),'|',('ttype','=','date'),"
                                                    "('ttype','=','datetime')]",
                                             string=" تصفية تاريخ Kpi")

    ks_item_start_date_2 = fields.Datetime(string="تاريخ بدء Kpi")
    ks_item_end_date_2 = fields.Datetime(string="تاريخ انتهاء Kpi")

    ks_domain_2 = fields.Char(string="مجال Kpi")
    ks_domain_2_temp = fields.Char(string="بديل مجال Kpi")
    ...
    ks_date_filter_selection_2 = fields.Selection([
        ...
        ('n_futurestarting_tomorrow', 'المستقبل يبدأ غدا'),
        ('l_custom', 'تصفية مخصص'),
    ], string="تحديد مرشح تاريخ Kpi", required=True, default='l_none')

    ks_previous_period = fields.Boolean(string=" قارن مع الفترة السابقة ", help='Checkbox to show comparison between the data of present day and the previous selected period. ')

    # ------------------------ Pro Fields --------------------
    ks_dashboard_mtn_board_id = fields.Many2one('ks_dashboard_mtn.board', string="لوحة المعلومات",
    ...
    ks_dashboard_item_type = fields.Selection([('ks_tile', 'Tile'),
                                               ('ks_bar_chart', 'مخطط شريطي'),
                                               ('ks_horizontalBar_chart', 'مخطط شريطي أفقي'),
                                               ('ks_line_chart', 'مخطط بياني خطي'),
                                               ('ks_area_chart', 'مخطط المساحة'),
                                               ('ks_pie_chart', 'مخطط دائري'),
                                               ('ks_doughnut_chart', 'الرسم البياني الدائري المجوف'),
                                               ('ks_polarArea_chart', 'مخطط المنطقة القطبية'),
                                               ('ks_list_view', 'عرض القائمة'),
                                               ('ks_kpi', 'مؤشر الأداء'),
                                               ('ks_to_do', 'To Do')
    ...
    ks_chart_relation_groupby = fields.Many2one('ir.model.fields',
                                                ...
                                                string="مجموعة من", help=' Define the x-axis of the graph. ')
    ks_chart_relation_sub_groupby = fields.Many2one('ir.model.fields',
                                                    domain="[('model_id','=',ks_model_id),('name','!=','id'),('name','!=','sequence'),"
                                                           "('store','=',True),('ttype','!=','binary'),"
                                                           "('ttype','!=','many2many'), ('ttype','!=','one2many')]",
                                                    string=" المجموعة الفرعية حسب",
                                                    help='Select the second level of grouping. ')
    ks_chart_date_groupby = fields.Selection([('minute', 'دقيقة'),
                                              ('hour', 'ساعة'),
                                              ('day', 'يوم'),
                                              ('week', 'أسبوع'),
                                              ('month', 'شهر'),
                                              ...
                                              ('year', 'سنة'),
                                              ('month_year', 'شهر-سنة')
                                              ], string="تجميع مخطط عنصر لوحة المعلومات حسب النوع")
    ks_chart_date_sub_groupby = fields.Selection([('minute', 'دقيقة'),
                                                  ('hour', 'ساعة'),
                                                  ('day', 'يوم'),
                                                  ('week', 'اسبوع'),
                                                  ('month', 'شهر'),
                                                  ('quarter', 'ربع'),
                                                  ('year', 'سنة'),
                                                  ], string="تجميع فرعي للمخطط عنصر لوحة المعلومات حسب النوع")
    ks_graph_preview = fields.Char(string="معاينة الرسم البياني", default="Graph Preview")
    ks_chart_data = fields.Char(string="بيانات المخطط في شكل سلسلة", compute='ks_get_chart_data', compute_sudo=False)
    ks_chart_data_count_type = fields.Selection([('count', 'العد'), ('sum', 'المجموع'), ('average', 'المتوسط')],
                                                string="نوع البيانات", default="sum")
    ks_chart_measure_field = fields.Many2many('ir.model.fields', 'ks_dn_measure_field_rel', 'measure_field_id',
                                              ...
                                              domain="[('model_id','=',ks_model_id),('name','!=','id'),('name','!=','sequence'),"
                                                     ...
                                                     "('ttype','=','integer'),('ttype','=','float'),"
                                                     "('ttype','=','monetary')]",
                                              string="قياس 1", help='Data points to be selected.')
    ...
    ks_chart_cumulative_field = fields.Many2many('ir.model.fields', 'ks_dn_cumulative_measure_field_rel',
                                                 ...
                                                 domain="[('model_id','=',ks_model_id),('name','!=','id'),('name',"
                                                        ...
                                                        "('ttype','=','integer'),('ttype','=','float'),"
                                                        "('ttype','=','monetary')]",
                                                 string="حقل تراكمي", help='Data points to be selected.')
    ...
    ks_bar_chart_stacked = fields.Boolean(string="رسم بياني شريطي مكدس", help='Stack the columns of the same record. ')

    ks_semi_circle_chart = fields.Boolean(string="مخطط نصف دائري")
    ...
    ks_sort_by_field = fields.Many2one('ir.model.fields',
                                       ...
                                       string="فرز حسب المجال", help='Select the desired sorting preference. ')
    ks_sort_by_order = fields.Selection([('ASC', 'تصاعدي'), ('DESC', 'تنازلي')],
                                        string="امر ترتيب", help=' Select the order of the sorting. ')
    ks_record_data_limit = fields.Integer(string="حد التسجيل", help=' Records to be displayed on the graph')

    ks_list_view_preview = fields.Char(string="معاينة طريقة العرض على شكل قائمة", default="List View Preview")

    ks_kpi_preview = fields.Char(string="Kpi Preview", default="KPI Preview")

    ks_kpi_type = fields.Selection([
        ('layout_1', 'KPI مع الهدف'),
        ('layout_2', 'مقارنة البيانات'),
    ], string="تخطيط Kpi", default="layout_1")

    ks_target_view = fields.Char(string="عرض", default="Number", help=' Select the view to compare target with data.')

    ks_data_comparison = fields.Char(string="نوع بيانات Kpi", default="None")

    ks_kpi_data = fields.Char(string="بيانات KPI", compute="ks_get_kpi_data", compute_sudo=False)
    ...
    ks_list_view_type = fields.Selection([('ungrouped', 'Un-Grouped'), ('grouped', 'Grouped')], default="ungrouped",
                                         string="نوع عرض القائمة", required=True,
    ...
    ks_list_view_fields = fields.Many2many('ir.model.fields', 'ks_dn_list_field_rel', 'list_field_id', 'field_id',
                                           ...
                                           string="الحقول المراد عرضها في القائمة",
    ...
    ks_company_id = fields.Many2one('res.company', string='الشركة', default=lambda self: self.env.user.company_id,
    ...
    ks_goal_enable = fields.Boolean(string="تمكين الهدف", help='Show the set target.')
    ks_goal_bar_line = fields.Boolean(string="إظهار الهدف كخط")
    ks_standard_goal_value = fields.Float(string="الهدف القياسي", help='Show the set target')
    ks_goal_lines = fields.One2many('ks_dashboard_mtn.item_goal', 'ks_dashboard_item', string="خطوط الهدف")
    ...
    ks_show_data_value = fields.Boolean(string="إظهار قيمة البيانات", help=' Display value on the graph. . ')

    ks_action_lines = fields.One2many('ks_dashboard_mtn.item_action', 'ks_dashboard_item_id', string="خطوط العمل")

    ks_actions = fields.Many2one('ir.actions.act_window', domain="[('res_model','=',ks_model_name)]",
                                 string="أجراءات", help="Redirects you to the selected view. ")

    ks_compare_period = fields.Integer(string="تشمل الفترة",
                                       help=' Provide the number of Date Filter Selection you want to include while displaying the record.')
    ks_year_period = fields.Integer(string="نفس الفترة السنوات السابقة",
                                    help=' Display the record for the same Date field for the last year. ')
    ks_compare_period_2 = fields.Integer(string="تضمين فترة KPI")
    ks_year_period_2 = fields.Integer(string="KPI Same Period Previous Years")

    ks_multiplier_active = fields.Boolean(string="تطبيق المضاعف", default=False,
                                        help="Provides the visibility of multiplier field")
    ks_multiplier = fields.Float(string="المضاعف",default=1, help="Provides the multiplication of record value")
    ...
    ks_update_items_data = fields.Selection([
        ...
        ('300000', '5 دقائق'),
        ('600000', '10 دقائق'),
    ], string="الفاصل الزمني لتحديث العنصر", default=lambda self: self._context.get('ks_set_interval', False),
    ...
    ks_unit_selection = fields.Selection([
        ('monetary', 'نقدي'),
        ('custom', 'العادة'),
    ], string="حدد نوع الوحدة", help='Select the unit to be assigned to the value. ')
    ...
    ks_show_records = fields.Boolean(string="إظهار السجلات", default=True, help="""This field Enable the click on 
                                                                                  Dashboard Items to view the Odoo 
                                                                                  default view of records""")
    #  Field for fill temp data
    ks_fill_temporal = fields.Boolean('املأ القيمة الزمنية')
    # Domain Extension field
    ks_domain_extension = fields.Char('تمديد المجال', help="Define conditions for filter to write manually")
    ks_domain_extension_2 = fields.Char('KPI Domain Extension')
    # hide legend
    ks_hide_legend = fields.Boolean('عرض', help="Hide all legend from the chart item", default=True)
    ...
    ks_data_format = fields.Selection([
        ('global', 'تنسيق اللغة الإنجليزية'),
        ('indian', 'التنسيق الهندي'),
        ('colombian', 'تنسيق البيزو الكولومبي-كولومبيا'),
        ('exact', 'القيمة الدقيقة')],
        string='تنسيق الرقم',
        default='global',
        help="لتغيير تنسيق الأرقام الذي يظهر في الرسم البياني لخيار معين")
    ks_button_color = fields.Char(string="لون الزر العلوي",
                                  default="#000000,0.99")
    ks_auto_update_type = fields.Selection(
        [('ks_live_update', 'التحديث في كل حالة.'), ('ks_update_interval', ' التحديث بعد الفاصل الزمني المحدد')],
        string='نوع التحديث التلقائي',
        default=lambda self: 'ks_update_interval' if self._context.get('ks_set_interval', False) else False,
        help='حدد نوع التحديث.')
    ks_show_live_pop_up = fields.Boolean(string='إظهار تحديث مباشر ', help='Checkbox to enable notification after every update. ')

    ks_is_client_action = fields.Boolean('إجراءات العميل', default=False)
    ks_client_action = fields.Many2one('ir.actions.client',
                                       string="إجراء عنصر العميل",
    ...
    ks_precision_digits = fields.Integer('أرقام', compute="_ks_compute_precision_digits", store=True, readonly=False)
    ...
    def _ks_compute_precision_digits(self):
        for rec in self:
            try:
                precision_digits = self.sudo().env.ref('ks_dashboard_mtn.ks_dashboard_mtn_precision')
    ...
    # default = lambda self: self.sudo().env.ref('ks_dashboard_mtn.ks_dashboard_mtn_precision')
    ...
    def copy_data(self, default=None):
        ...
        return super(KsDashboardmtnItems, self).copy_data(default)

    def copy(self, default=None):
        default = default or {}
        res = super(KsDashboardmtnItems, self).copy(default)
    ...
    def create(self, values):
        ...
        return super(KsDashboardmtnItems, self).create(
            values)
    ...
    def write(self, values):
        ...
        return super(KsDashboardmtnItems, self).write(
            values)
            ...
            if ((rec.ks_chart_data_count_type != "count" and ks_chart_measure_field) or (
                ...
                if (rec.ks_chart_groupby_type == 'date_type' and rec.ks_chart_date_groupby) or \
                    ...
                    if rec.ks_chart_groupby_type == 'date_type' and rec.ks_goal_enable and rec.ks_dashboard_item_type in [
                        ...
                        ks_goal_records = self.env['ks_dashboard_mtn.item_goal'].read_group(
    ...
    def ks_get_previous_period_data(self, rec):
        ...
        ks_date_data = False
        if rec.ks_date_filter_selection == "l_none":
            date_filter_selection = rec.ks_dashboard_mtn_board_id.ks_date_filter_selection
        else:
    ...
    def ks_get_start_end_date(self, model_name, ks_chart_groupby_relation_field, ttype, ks_chart_domain,
        ...
        #         self.env['ks_dashboard_mtn.item_goal'].search([('ks_goal_date', '>=', model_field_start_date.strftime("%Y-%m-%d")),
        ...
        goal_model_start_date = \
            self.env['ks_dashboard_mtn.item_goal'].search(ks_goal_domain, limit=1,
                                                            order='ks_goal_date ASC')['ks_goal_date']
        goal_model_end_date = \
            self.env['ks_dashboard_mtn.item_goal'].search(ks_goal_domain, limit=1,
                                                            order='ks_goal_date DESC')['ks_goal_date']
...
class KsDashboardItemsGoal(models.Model):
    _name = 'ks_dashboard_mtn.item_goal'
    _description = 'Dashboard mtn Items Goal Lines'

    ks_goal_date = fields.Date(string="تاريخ")
    ks_goal_value = fields.Float(string="قيمة")

    ks_dashboard_item = fields.Many2one('ks_dashboard_mtn.item', string="عنصر لوحة المعلومات")


class KsDashboardItemsActions(models.Model):
    _name = 'ks_dashboard_mtn.item_action'
    _description = 'Dashboard mtn Items Action Lines'
    ...
    ks_item_action_date_groupby = fields.Selection([('minute', 'دقية'),
                                                    ('hour', 'ساعة'),
                                                    ('day', 'يوم'),
                                                    ('week', 'أسبوع'),
                                                    ('month', 'شهر'),
                                                    ('quarter', 'ؤبع'),
                                                    ('year', 'سنة'),
                                                    ], string="تجميع حسب التاريخ")

    ks_chart_type = fields.Selection([('ks_bar_chart', 'مخطط شريطي'),
                                      ('ks_horizontalBar_chart', 'مخطط شريطي أفقي'),
                                      ('ks_line_chart', 'مخطط بياني خطي'),
                                      ('ks_area_chart', 'مخطط المساحة'),
                                      ('ks_pie_chart', 'مخطط دائري'),
    ...
    ks_dashboard_item_id = fields.Many2one('ks_dashboard_mtn.item', string="عنصر لوحة المعلومات")
    ...
    ks_record_limit = fields.Integer(string="حد التسجيل")
    ks_sort_by_field = fields.Many2one('ir.model.fields',
                                       domain="[('model_id','=',ks_model_id),('name','!=','id'),('name','!=','sequence'),('store','=',True),"
...
class KsDashboardItemMultiplier(models.Model):
    ...
    _description = 'Dashboard mtn Items Multiplier Lines'

    ks_dashboard_item_id = fields.Many2one('ks_dashboard_mtn.item', string="عنصر لوحة المعلومات")
    ks_model_id = fields.Many2one('ir.model', related='ks_dashboard_item_id.ks_model_id')
    ks_multiplier_value = fields.Float(string="المضاعف", default=1)
